...
const TodoTemplate = () => {
  ...
  //자식 컴포넌트가 부모 컴포넌트에게 데이터르 전달할 때는
  //props 사용이 불가능.
  ...
  const removeTodo = id => {
    ...
    //주어진 배열의 값들을 순회하여 조건의 맞는 요소들만 모아서
    //새로운 배열로 리턴해 주는 함수.
    setTodos(todos.filter(todo => todo.id !== id));

    fetch(API_BASE_URL + '/' + id, { //`${API_BASE_URL}/${id}` 로 적어도 됨.
      method : 'DELETE',
      headers : { 'content-type' : 'application/json' }
    })
    .then(res => res.json())
  ...
  const countRestTodo = () => {
    //방법1 : 내가 쓴 로직 (오답 아님)
    // let count =0; 
    // todos.map(todo => !todo.done ? count++ : count);
    // return count;
  ...
  useEffect(() => {
    ...
    //페이지가 랜더링 됨과 동시에 할 일 목록을 요청해서 뿌려 주겠습니다.
    fetch(API_BASE_URL)
      .then(res => res.json())
      .then(json => {
